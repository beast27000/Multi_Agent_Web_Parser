Step 8, File 1: LangGraph Agent Orchestrator
Concept
LangGraph uses a StateGraph to define agent roles as nodes. Each node is a function/runnable that processes state and returns updates. Edges connect nodes in a DAG (directed acyclic graph). Perfect for our 6-agent pipeline where state flows through IntentExtractor → SearchPlanner → MultiSiteFetcher → ChunkProcessor → CrossSiteRanker → FinalSynthesizer.
...
Key Concepts
Concept	Description
StateGraph	LangGraph primitive for DAG-based agents; nodes are functions, edges define flow
AgentState	TypedDict accumulates outputs from all 6 agents; passed through graph
Command(update=..., goto=...)	Node returns Command to update state AND route to next node
6 Agents as Nodes	IntentExtractor, SearchPlanner, MultiSiteFetcher, ChunkProcessor, CrossSiteRanker, FinalSynthesizer
Error Handling	Each agent can route to END on error via Command(goto=END)
Async/Await	All agents use asyncio for concurrent operations (search, fetch, etc.)
How It Connects
IntentExtractor parses raw query → outputs IntentSchema
SearchPlanner reads intent → uses SearchTool → outputs search_results
MultiSiteFetcher reads search results → uses URLFetcher → outputs fetched_html
ChunkProcessor reads HTML → runs ETLPipeline → outputs chunks
CrossSiteRanker reads chunks → uses RankCompareTool → outputs ranked_chunks
FinalSynthesizer reads ranked chunks → creates final_answer with provenance